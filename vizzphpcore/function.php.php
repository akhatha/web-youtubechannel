...
//checking user laready exist based on username ,email and pwd	
function forgot_user_exist($username)
{
			$pass=md5($password);
			$query=mysql_query("SELECT * FROM  users WHERE (channel_name='$username' OR email='$username')");
			$user=	mysql_fetch_assoc($query);
			$username= $user['channel_name'];
			if(isset($username))
			{
...
function forgot_user_data($username)
{
			$pass=md5($pass);
			$query=mysql_query("SELECT * FROM  users WHERE (channel_name='$username' OR email='$username')");
			$user=	mysql_fetch_assoc($query);
...
function sendForgotMail($to_email,$user_name,$user_email,$pass,$message){
    ...
    $subject="Vizzdeo Forgot password";
