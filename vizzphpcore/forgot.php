...
{
$email=$_POST['email'];
$userExist=forgot_user_exist($email);//cheking user already exist or not
	$mess=null;
if($userExist)
	{
	$id=forgot_user_data($email);
	$pass=rand_string(8);
	$data=array("password"=>md5($pass));
	updateName('users',$data,$id);
	sendForgotMail($email,'VIZZDEO',$email,$pass,"Vizzdeo Forgot Mail");
	
                $mess="<div class='alert alert-success fade in' id='success'>
						<strong>Success!</strong> Mail sent Sucessfully.Please check your Email address
						</div>";;
				header("Refresh: 1;log_in_page.php");//if user exist it redirects to dashboard
...
?></div>
				<div class="form-group label-floating">
					...
					<input required="" placeholder="abc@abc.com" class="form-control" placeholder="" type="email" name="email">
				</div>
				
                
				
				<!--<a href="01-home.html" class="btn btn-lg btn-primary full-width">Login</a>-->
<input class="btn btn-lg btn-primary full-width" type="submit" name="submit" value="Forgot" required>
